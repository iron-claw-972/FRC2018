...
public class SplineGeneration {
    ...
    public static Trajectory[] generateWheelTrajectories(Trajectory trajectory_input, double robot_width_wheels) {
        ...
        for (int i = 0; i < trajectory_output[0].segments_.length; i++) {
            leftVelocity.add(trajectory_output[0].segments_[i].vel);
            rightVelocity.add(trajectory_output[1].segments_[i].vel);
            traj.add(trajectory_input.segments_[i].vel);
        }
    ...
    public static Trajectory generateSpline(double velo, double accel, PointsPath pointsPath, double dt) {
        ...
        Trajectory traj = TrajectoryGeneration.generate(velo, accel, total_distance, pointsPath.path.get(0).heading, pointsPath.path.get(pointsPath.path.size() - 1).heading, dt);
